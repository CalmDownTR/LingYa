...
from lingya.gateway.server import (
    OP_CLOSE,
    OP_PING,
    OP_PONG,
    OP_TEXT,
...
def _build_masked_frame(opcode: int, payload: bytes, mask: bytes | None = None) -> bytes:
...
class TestMaskedFrame:
    async def test_masked_frame_unmasked_correctly(self):
        ...
        frame = _build_masked_frame(OP_TEXT, payload, mask)
    ...
    async def test_masked_close_frame(self):
        ...
        frame = _build_masked_frame(OP_CLOSE, b"", mask)
    ...
    async def test_masked_ping_frame(self):
        ...
        frame = _build_masked_frame(OP_PING, ping_data, mask)
